...
import { CheckCircle2, XCircle, Minus, Clock, TrendingUp, ClipboardCheck, Calendar, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {sampleAuditData.project.name} — {currentMetrics.month}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          title="Compliance"
          value={`${currentMetrics.compliance}%`}
          subtitle={`+${currentMetrics.compliance - sampleAuditData.monthlyTrend[sampleAuditData.monthlyTrend.length - 2].compliance}% vs prev month`}
          icon={TrendingUp}
          variant="primary"
        />
        <MetricCard
          title="Compliant"
          value={currentMetrics.compliant}
        ...
        <MetricCard
          title="Non-Compliant"
          value={currentMetrics.nonCompliant}
          subtitle="Open NCs this period"
          icon={XCircle}
          variant="danger"
        />
        <MetricCard
          title="Noted / N/A"
          value={currentMetrics.noted}
          subtitle="Items not applicable"
          icon={Minus}
          variant="default"
        />
      </div>
              ...
              {sampleAuditData.monthlyTrend.slice().reverse().map((row, i) => (
                <tr key={row.month} className="border-b last:border-0 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="text-success font-medium">{row.compliant}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
